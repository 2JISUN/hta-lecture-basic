...
// SpaceShipGame 클래스는 게임 애플리케이션의 진입점을 정의합니다.
public class SpaceShipGame extends JFrame {
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> new SpaceShipGame());
    }
}
// ShipPanel 클래스는 게임의 주요 그래픽 컴포넌트를 관리하고 그리는 역할을 합니다.
class ShipPanel extends JPanel implements KeyListener, ActionListener {
    ...
    private Image love = new ImageIcon("images/love.png").getImage(); // 러브 이미지
    ...
    // 총알 발사 관련 변수 추가
    private boolean spaceKeyPressed = false; // 스페이스바 키가 눌렸는지 여부를 나타내는 변수
    private long lastBulletTime = 0; // 마지막 총알 발사 시간을 저장하는 변수
    private long bulletCooldown = 200; // 총알 발사 간격 (200ms)
    ...
    public void paintComponent(Graphics g) {
        ...
        // 러브 그리기
        int loveWidth = love.getWidth(null);
        int loveHeight = love.getHeight(null);
        int loveSpacing = 10; // 이미지 사이의 여백
        int totalLoveWidth = (loveWidth + loveSpacing) * 3;

        for (int i = 0; i < 3; i++) {
            int loveX = getWidth() - totalLoveWidth + (loveWidth + loveSpacing) * i;
            int loveY = 0;

            g.drawImage(love, loveX, loveY, null);
        }
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        // 스페이스바를 누르고 떼기 전까지 연속해서 총알 발사 처리
        long currentTime = System.currentTimeMillis();
        if (spaceKeyPressed && currentTime - lastBulletTime >= bulletCooldown) {
            bullets.add(new Bullet(shipX + ship.getWidth(null) / 2 - bullet.getWidth(null) / 2, shipY));
    ...
    public void keyPressed(KeyEvent e) {
        ...
        if (keyCode == KeyEvent.VK_LEFT) {
            ...
            spaceKeyPressed = true; // 스페이스바 키 눌림을 저장
        }
    }
    ...
    public void keyReleased(KeyEvent e) {
        ...
        if (keyCode == KeyEvent.VK_LEFT) {
            ...
            spaceKeyPressed = false; // 스페이스바 키 떼짐을 저장
        }
    }
